...
// add the boot to the /etc/profile if not already there
const profile = FS.readFileSync(`${IMAGE_MNT_ROOT}/etc/profile`, "utf-8")
if (!profile.includes("boot.ps1")) {
    execSync(
        `echo ${USER_PASSWD} | sudo -E -S ` +
        `bash -c "echo 'boot.ps1' >> ${IMAGE_MNT_ROOT}/etc/profile"`,
        {
            shell: "/bin/bash",
            stdio: "inherit",
            encoding: "utf-8",
...
if (!profile.includes("exit")) {
    execSync(
        `echo ${USER_PASSWD} | sudo -E -S ` +
        `bash -c "echo 'exit' >> ${IMAGE_MNT_ROOT}/etc/profile"`,
        {
            shell: "/bin/bash",
            stdio: "inherit",
            encoding: "utf-8",
